...
local redzlib = loadstring(game:HttpGet("https://raw.githubusercontent.com/tv7775464-netizen/Redzhub/refs/heads/main/RedzUI_Modified.lua"))()
...
local Tabs = {
    Info = Window:MakeTab({ Title = "discord", Icon = "info" }),
    ...
    Misc = Window:MakeTab({ Title = "misc", Icon = "rbxassetid://6031280882" })
}

Tabs.Info:AddSection("Information")

Tabs.Info:AddDiscordInvite({
    Title = "Discord - Redzhub",
    Description = "remeke thoi bro",
    Logo = "rbxassetid://15298567397",
    Invite = "discord.gg/Redzhub",
})
...
AttackDropdown = Tabs.Main:AddDropdown({
    ...
    Options = {"Normal Attack","Fast Attack"},
    Default = "Fast Attack",
    Callback = function(Value)
...
-- ========== FAST ATTACK (mẫu Fast 2 người dùng cung cấp) ==========
local FA_Modules = game.ReplicatedStorage.Modules
local FA_Net = FA_Modules:WaitForChild("Net")
local Register_Hit = FA_Net:WaitForChild("RE/RegisterHit")
local Register_Attack = FA_Net:WaitForChild("RE/RegisterAttack")

local function GetAllBladeHits()
    local bladehits = {}
    local character = game.Players.LocalPlayer.Character
    local root = character and character:FindFirstChild("HumanoidRootPart")
    if not root then return bladehits end

    for _, v in pairs(workspace.Enemies:GetChildren()) do
        local hum = v:FindFirstChild("Humanoid")
        local hrp = v:FindFirstChild("HumanoidRootPart")
        local head = v:FindFirstChild("Head")
        if hum and hrp and head and hum.Health > 0
...
local function Getplayerhit()
    ...
    for _, v in pairs(workspace.Characters:GetChildren()) do
        ...
        local hrp = v:FindFirstChild("HumanoidRootPart")
        local head = v:FindFirstChild("Head")
        if v.Name ~= game.Players.LocalPlayer.Name and hum and hrp and head and hum.Health > 0
        and (hrp.Position - root.Position).Magnitude <= 65 then
            table.insert(bladehits, v)
        end
    end
...
local function FastAttackSample()
    if _G.Seriality == false then return end
